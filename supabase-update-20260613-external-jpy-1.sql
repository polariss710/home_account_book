-- supabase-update-20260613-external-jpy-1.sql
-- Purpose:
-- - Add idempotent external-source JPY transaction support for Phase 1
--   aozora school personal-business teacher wage linkage.
...
create unique index if not exists home_jpy_transactions_external_idempotency_unique
  on public.home_jpy_transactions (external_idempotency_key)
  where created_by_external is true
    and external_idempotency_key is not null;

create unique index if not exists home_jpy_transactions_external_source_event_unique
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'home_jpy_transactions_external_required_check'
      and conrelid = 'public.home_jpy_transactions'::regclass
  ) then
    alter table public.home_jpy_transactions
...
begin
  ...
  if v_external_reference_type <> 'school_payment_requests' then
    return jsonb_build_object('ok', false, 'message', 'external_reference_type must be school_payment_requests');
  end if;
  ...
  if v_external_event_type = 'teacher_wage_payment_confirm' and v_transaction_type <> 'expense' then
    return jsonb_build_object('ok', false, 'message', 'teacher_wage_payment_confirm must create an expense transaction');
  end if;

  if v_external_event_type = 'teacher_wage_payment_reverse' and v_transaction_type <> 'income' then
    return jsonb_build_object('ok', false, 'message', 'teacher_wage_payment_reverse must create an income transaction');
  end if;

  if v_external_event_type not in ('teacher_wage_payment_confirm', 'teacher_wage_payment_reverse') then
...
comment on function public.home_create_external_jpy_transaction(
  ...
  'Creates one idempotent external-source JPY transaction for aozora school personal-business teacher wage linkage. Does not write CNY, FX, fixed items, or school DB.';
