...
--
-- 2. **信用卡 statement 关联项的处理保持原样。** 那是另一类对象，Phase 3E 的账单
--    确认链路会动它们的金额，不是「只改状态」这么简单，不能照搬本文件的跳过逻辑。
--
--    ⚠️ **但两边的机制不同，别当成同一条**（2026-09-04 审核订正）：
--      日元  有前置检查 home_fixed_scope_has_card_statement，整批拒绝并返回
--            HOME_CARD_STATEMENT_ITEM_BULK_STATUS_FORBIDDEN，一行未改
--      人民币 **没有**对应的前置检查。它的 UPDATE 会被生产触发器抛出的
--            HOME_CARD_STATEMENT_ITEM_UPDATE_FORBIDDEN 拒绝，**事务回滚**
--    结果都是拒绝，但一个是函数里的返回、一个是触发器抛异常，前端看到的形状不同。
--
