...
--
-- ===========================================================================
-- 已部署 —— 2026-09-04 09:35 JST
-- ===========================================================================
--
-- 执行成功并 COMMIT。回填输出 UPDATE 1（目标 3b926e75… 写入
-- original_amount=166100 / original_currency=JPY，其余字段未变）。
-- Correction-P 那条与 52 条 immediate 请求的两列仍为 NULL；54 条请求排除新增两列
-- 后的逐行哈希全部未变。五个函数部署后的 owner / DEFINER / proacl 与部署前一致。
--
-- 第零节四条证伪用例全部按预期 P0001 中止；第五节除下述两项外全部按预期错误码
-- 失败；第三、四、六、七节回归通过；第八节工行卡 CNY fixture 三项实测通过后已回滚，
-- 残留检查为 0。
--
-- ---------------------------------------------------------------------------
-- ⚠️ 未验证的两项：第五节的 f 与 g（函数级）
-- ---------------------------------------------------------------------------
--
--   f. 批准时 request.original_amount 为 NULL → 期望 CONTRACT_INVALID
--   g. 同币种但 original_amount ≠ amount → 期望 CONTRACT_INVALID（函数级）
--
-- **这两项未执行，不是「通过」。** 要构造它们需要的坏行被本文件自己的表级 CHECK
-- 与 route 触发器挡住，而临时摘掉生产上的这两道守卫，业务负责人 2026-09-04
-- 明确不批准。
--
-- 判断依据：f/g 验的是**冗余**而非行为。批准核心里那四条原币校验是**故意加的
-- 第三层**（见第 10 步 ① 处注释），万一写错的失效模式是「三层退化成两层」，
-- 不是「放行错误数据」。而那两层都已实测：
--
--   第一层 表级 CHECK      → g 的表级形态实测 23514
--   第二层 route 触发器冻结 → d 实测 42501 FIXED_CARD_REQUEST_EVIDENCE_IMMUTABLE
--
-- 关于第三层能给出的证据，只有一句：第四节那次 JPY fixture **批准成功**，说明
-- 契约检查这条 if 链会被求值且不会误报（否则合法请求就批不过去）。
-- **没有任何证据表明原币那四条子句能够触发。** 残余风险就是它们写成了恒假。
--
--   订正（2026-09-04）：本节初稿写过「k（SCHEDULE_MISMATCH）能触发，说明控制流
--   走过了契约检查」。**这条依据是错的**——k 实测返回的是
--   HOME_FIXED_REQUEST_SCHEDULE_MISMATCH，那是**创建器**的错误码，不是批准核心的
--   HOME_FIXED_APPROVAL_SCHEDULE_MISMATCH。两个函数有同名检查，我搞混了。
--   由审核在 2026-09-04 补正。留下这段是因为「拿错证据支撑一个未验结论」比结论
--   本身更值得记。
--
-- **接手的人注意**：不要因为本文件写着「已部署、验收通过」就认为第五节全验了。
-- 若将来放宽了那两道守卫、或有了可用的 staging 环境，f/g 应当补验。
-- （现有的 codex/cash-staging-environment 是 2026-07-18 建的 V3 环境，
--   schema 落后六周以上，且文档明确禁止它指向 V2 生产，不适用。）
--
-- ===========================================================================
