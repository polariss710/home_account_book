...
- Start each work turn by reading `docs/lessons.md`. 该文件记录已经导致过返工或事故的具体教训，每条都是可执行的检查动作，附出处与日期。涉及权限/ACL、磁盘 SQL 与生产的一致性、复用既有函数、删除相关逻辑时，先对照该文件再动手。踩到新的同类坑时追加条目；条目被证伪时划掉而不是删除，删掉等于把踩坑过程也抹了。
