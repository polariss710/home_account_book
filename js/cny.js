...
import { appState, findCnyAccount } from "#state";
import {
  deleteAccount,
...
function renderTransactions() {
  const transactions = appState.cnyPage?.transactions || [];
...
function transactionRow(item) {
  return `
    <tr>
      ...
      <td><input class="table-input amount-input" data-cny-amount="${item.id}" type="number" step="1" value="${Number(item.amount || 0)}" /></td>
