-- ###########################################################################
-- ##  配套依赖：必须在 supabase-update-20260905-card-route-catalog-left-join.sql
-- ##  **之后**部署。顺序固定，不是「不限」。
-- ##
-- ##  理由：home_get_card_route_catalog 原本用 INNER JOIN 连模板表。本文件解除
-- ##  NOT NULL 之后，没有家庭模板的卡会从那个运维目录里整行消失（审核 P2）。
-- ##
-- ##    先上 LEFT JOIN     → 列仍 NOT NULL，行为与今天完全相同，无害
-- ##    先上本文件         → 立刻出现漏卡窗口
-- ##
-- ##  两个文件都已通过 2026-09-05 的代码复审。School 侧的卡列表直读
-- ##  home_card_instruments，不经过那个目录，不受影响。
-- ###########################################################################
