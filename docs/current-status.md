...
- Cash dev version `20260718-cash-dev-v3-6` is live on the isolated Render static site. It groups two or more pending canonical `school_expense_records / expense_paid` teacher-wage requests only when teacher, business month, currency, Cash account and payment date are identical; single requests stay on the ordinary row action path.
...
- `home_reject_teacher_wage_request_group(uuid[], text)` locks and rejects the whole pending wage group in one Cash transaction. Any status/group mismatch returns zero writes and no Cash transaction; exact retries are idempotent. School callbacks remain per canonical expense so a failed callback can be retried by the existing rejected-row recovery action without changing Cash again.
- Pending group cards now display Cash account and payment date. Batch audit cards load the underlying request snapshots and show business attribution, full request/expense/transaction identities, confirmation time and School sync time.
...
- Real atomic-rejection E2E passed in the live v3-6 UI with `勤务表跨业务测试老师 / 2026-12`: JPY 12,300 personal plus JPY 18,700 青空进学塾 were rejected together with the same `E2E atomic group rejection` reason and written back to School as `cash_rejected`. Both Cash request transaction IDs and both School transaction IDs remain null, the related JPY/CNY Cash transaction counts are zero, and replaying one School callback returned an idempotent success.
- User-facing verification of the live aggregate audit card passed on 2026-07-18: batch total, two-item mapping, Cash and School batch identities, transaction identity, sync state and expandable detail display were all confirmed correct.
- Rollback verification passed for exact approval retry, exact School marker retry, conflicting marker rejection, unique transaction count, aggregate transaction update/delete guards, atomic rejection, rejection retry and mismatched-date zero-write behavior. This remains dev-only until separately migrated and tested in staging/prod.
