...
- Phase 3B is deployed as a database-only foundation for future School credit-card fixed payments. It separates card instruments, physical funding payment channels, monthly statement cycles, and School fixed-payment projections. The sole production card master is `西武卡` (`9b27347e-2dce-4caf-bac0-67f053ef6c3b`), funded through the existing `邮局卡` channel and bound to the existing household `西武卡消费` template; the card is active while its School fixed route remains disabled.
...
- Cash UI version `20260819-accounting-scope-filter-2` renders canonical external rows as read-only, keeps the `School同步生成` badge, exposes no edit/copy/delete controls, and adds accounting-scope labels/filters. CNY display uses two decimal places (`1120.50` -> `1,120.50 CNY`); JPY remains zero-decimal.
...
- Backfilled all 50 existing `home_external_transaction_requests` rows to `payment_route = immediate_account` and `projection_status = not_applicable`; added guarded fixed-route metadata without changing the existing create/approve/reject writer definitions or signatures. The production count remains 42 approved/mapped, 6 pending, and 2 rejected.
- Installed the active `西武卡` instrument with cutoff day 10 inclusive, funding day 25, funding channel `邮局卡`, household statement template `西武卡消费`, and `is_school_fixed_route_enabled = false`. No statement cycle, external fixed projection, School fixed item, funding transaction, compensation transaction, or fixed-route request was created.
- Added owner-only pure helpers `home_calculate_card_fixed_schedule(uuid,date)` and `home_get_card_route_catalog(uuid)`. Both are security invoker with fixed `search_path = pg_catalog, public`; direct execution is revoked from PUBLIC, anon, authenticated, and service_role.
- Database triggers enforce same-user/same-currency card bindings, household-template scope, legacy immediate-route defaults, disabled fixed-route fail-closed behavior, projection identity/amount/month invariants, and column-level projection immutability while allowing future versioned status fields to change through a narrow writer.
- Final 2099 rollback rehearsal passed cutoff-before/on/after, cross-year, month-end clamping, invalid bindings, duplicate card/cycle/projection, invalid route combinations, old-writer compatibility, ACL denial, projection immutability, status whitelist, and fixture cleanup. The committed migration used the unchanged SHA-256 `d1d70888ea6e79d3d1c677a2dbb0efa1ee08cf0cbb722bcea60c17128dba1375`.
- Independent postcommit verification passed. All pre-existing table business fingerprints, account balances, five reader outputs, 46 existing home function definitions/owners/ACLs/signatures, the Phase 2 accounting-scope contract, the two external immutable triggers, `2026-08 西武卡消费 = 119,699`, and `邮局卡 = 340,134 / rounded 341,000` remained unchanged. The existing `202,991 JPY` classroom-rent request and transaction remain unchanged on `日元现金`.
- No group-funding/allocation table was created. That decision remains deferred until the Phase 3E writer contract demonstrates a need for a lightweight funding header.
...
- Responsive QA covers 1440, 1024, 768, and 390px. The mobile shell/sidebar/nav are explicitly width-clamped so the 390px document width remains equal to the viewport; wide tables continue to scroll only inside their table wrappers.
...
- No HTML/CSS/JS, reader definition, RPC signature, balance algorithm, annual-statistics rule, fixed settlement, fixed advance, or School V2 behavior changed in Phase 2A-P itself. Phase 2B1 later added reader/UI display filtering only; Phase 3B later added a disabled database foundation, while Phase 2B2+, executable Phase 3 routes, and Phase 4 remain deferred.
