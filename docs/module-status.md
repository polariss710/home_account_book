...
| Credit-card fixed route | Phase 3C3-B request-entry foundation deployed. Home DB owns card schedule calculation and an independent fixed pending-request writer; request/funding accounts are null, generic approve fails closed, and rejection has no downstream facts. `西武卡` remains active with School route disabled; fixed request/cycle/projection/School item counts are 0 | Phase 3D may add a dedicated fixed approval writer only after separate authorization; do not enable either fixed Gate here |
