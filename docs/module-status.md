...
| Credit-card fixed route | Phase 3E statement preview/confirm/reopen foundation deployed. DB computes ordered School manifest/subtotal and household remainder, updates only the bound household item, and records append-only revisions under optimistic version/idempotency and the shared card/month lock. No ledger, balance or funding write occurs. `西武卡` route remains disabled; fixed request/projection/cycle/revision/School item counts are 0 | Phase 3F may design 25-day group funding/allocation separately; do not enable either fixed Gate or add UI here |
