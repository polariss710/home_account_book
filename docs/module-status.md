...
| Teacher wage aggregate | Same-teacher/month/currency/account/date canonical wage requests can be approved as one Cash transaction or rejected atomically with zero Cash transactions; approval/rejection retries are idempotent, batch/item audit details are visible, and the 2026-07-18 JPY 59,100 approval E2E plus rejection rollback suite passed | Complete v3-6 static deployment/browser rejection check; then reproduce schema, callback config and E2E only during staging/prod rollout |
