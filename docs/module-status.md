...
| Teacher wage aggregate | Same-teacher/month/currency/account/date canonical wage requests can be approved as one Cash transaction or rejected atomically with zero Cash transactions; approval/rejection retries are idempotent, batch/item audit details are visible, and the 2026-07-18 JPY 59,100 approval plus JPY 31,000 live v3-6 atomic-rejection E2E passed | Keep the verified dev identities immutable; reproduce schema, callback config and E2E only during staging/prod rollout |
