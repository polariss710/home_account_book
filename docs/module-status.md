...
| Credit-card fixed route | Phase 3D approval/projection foundation deployed. Dedicated authenticated approval atomically creates one unpaid School fixed item plus one immutable projection and approved request; service-only evidence supports School callback. No transaction, balance or funding write occurs. Generic approval and all projected-item mutation paths fail closed. `西武卡` remains active with School route disabled; fixed request/cycle/projection/School item counts are 0 | Open neither Gate until a separately authorized activation phase proves the full live request/approval/sync path |
