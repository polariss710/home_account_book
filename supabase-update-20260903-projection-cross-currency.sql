-- ###########################################################################
-- ##  作废，禁止执行 —— 2026-09-03 审核驳回（随三步方案一并作废）
-- ##
-- ##  本文件自身的两条约束改动经审核认定正确，且三个 projection 守卫里确实
-- ##  没有隐藏的「原币 = 结算币」重复检查（攻击点 3 通过）。
-- ##
-- ##  但它属于被驳回的三步分事务方案，且缺一条：
-- ##  home_validate_external_fixed_projection 目前只核对
-- ##  request.amount/currency = projection.settlement_*，
-- ##  放开跨币种后还须补 request.original_* = projection.original_*，
-- ##  否则 request 与 projection 的原币事实仍可分叉。
-- ##
